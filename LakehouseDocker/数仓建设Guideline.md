...
报错
```
starrocks error: StarRocks planner use long time 3601 ms in memo phase, This probably because 1. FE Full GC, current query allocated=48.5 MiB, optimizerContextInUse=24.2 MiB2. Hive external table fetch metadata took a long time, 3. The SQL is very complex. You could 1. adjust FE JVM config, 2. try query again, 3. enlarge new_planner_optimize_timeout session variable
```

原因：从OSS拉取Paimon元数据花了比较长的时间（~15s）
