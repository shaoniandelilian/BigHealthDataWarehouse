...
CREATE TEMPORARY TABLE kafka_sensor_realtime (
    ...
    -- 姿态（秒级）：sit/stand/lie
    `posture`          STRING,
    `event_time` AS TO_TIMESTAMP_LTZ(`event_ts` / 1000.0, 3),
...
CREATE TEMPORARY TABLE kafka_sensor_history (
    ...
    `event_time` AS TO_TIMESTAMP_LTZ(`ts_start` / 1000.0, 3),
...
SELECT
    device_id,
    ts_start, ts_end,
    TO_TIMESTAMP_LTZ(ts_start / 1000.0, 3)                              AS event_time,
    DATE_FORMAT(TO_TIMESTAMP_LTZ(ts_start / 1000.0, 3), 'yyyy-MM-dd')   AS ds,
    DATE_FORMAT(TO_TIMESTAMP_LTZ(ts_start / 1000.0, 3), 'HH')           AS hh,
    heart_rate_min, rmssd, sdnn, pnn50, lf_hf_ratio,
    spo2, resp_rate, skin_temp,
    steps_min, activity_level_min, posture_min, wearing_ratio,
    CASE
