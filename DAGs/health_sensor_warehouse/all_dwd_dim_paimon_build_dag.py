...
SELECT od.id, od.order_id, oi.user_id, od.sku_id, oi.province_id, od.source_type, od.order_price, od.sku_num,
       (od.order_price * od.sku_num) AS split_original_amount, COALESCE(act.split_activity_amount, 0.0) AS split_activity_amount,
       COALESCE(cou.split_coupon_amount, 0.0) AS split_coupon_amount, od.split_total_amount, oi.order_status, od.create_time
FROM laife_stream.ods_order_detail od
JOIN laife_stream.ods_order_info oi ON od.order_id = oi.id
LEFT JOIN laife_stream.ods_order_detail_activity act ON od.id = act.order_detail_id
LEFT JOIN laife_stream.ods_order_detail_coupon cou ON od.id = cou.order_detail_id;
...
SELECT id, login_name, nick_name, pass_wd as name, phone_num, email, user_level, birthday, gender, create_time, operate_time
