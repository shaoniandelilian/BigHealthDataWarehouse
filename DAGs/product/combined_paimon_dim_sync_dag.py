...
-- 手动为 Paimon 目标预建表 (带有 NOT ENFORCED 主键以支持 Bucket)
CREATE TABLE IF NOT EXISTS laife_stream.ods_base_province (
  id BIGINT,
  name STRING,
  region_id STRING,
  area_code STRING,
...
-- 手动为 Paimon 目标预建表
CREATE TABLE IF NOT EXISTS laife_stream.ods_base_category1 (
  id BIGINT,
  name STRING,
  PRIMARY KEY (id) NOT ENFORCED
);
